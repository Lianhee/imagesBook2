class BookViewer {
	...
	async loadImages() {
		try {
			// Realizamos una solicitud fetch para obtener el archivo JSON
			const response = await fetch('imagenes.json') // Ajusta la ruta a tu archivo JSON
			const data = await response.json()

            console.log(data);

			// Verificamos si el archivo JSON tiene el campo 'images' con URLs válidas
			if (data.images && Array.isArray(data.images)) {
				...
				// Renderizar la imagen y las miniaturas
				this.render()
				this.updateThumbnails()
			} else {
				console.error('No se encontraron imágenes válidas en el archivo JSON.')
			}
		} catch (error) {
